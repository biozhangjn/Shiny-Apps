...
gene_names_df <- readRDS("./data/GeneIDs_DanRer10_ens85.RDS")
# ========
...
ui <- fixedPage(theme = shinytheme("paper"),
  ...
  headerPanel("Zebrafish Neuromast scRNA-seq"),
      ...
      tabsetPanel(
        tabPanel("Welcome!",
          fluidRow(
            ...
            column(12, tags$br()),
            column(10, 'Thank you for visiting the companion app for 
              our manuscript "Single-cell RNA-Seq reveals distinct stem cell
              populations that drive sensory hair cell regeneration in 
              response to loss of Fgf and Notch signaling". The purpose of this 
              app is to provide a simple enviroment for visualizing 
              gene expression in our single cell RNA-seq data set.
              One simply needs to enter a list of zebrafish genes
              in the sidebar and hit "Generate Plots" to visualize
              expression values on a t-SNE, violin, or
              heatmap plot in each tab listed above. 
              Please read the instructions below to orient 
              yourself with the type of input allowed when querying 
              genes and features. Also, feel free to download a
              pdf of the embedded image below for reference, as
              each cluster number in the violin and heatmap plots
...
# start R session
# rsconnect::deployApp('/Volumes/projects/ddiaz/Analysis/Scripts/rsconnect/shinyapps.io/neuromast_homeostasis_scRNAseq_2018', account = 'piotrowskilab')
